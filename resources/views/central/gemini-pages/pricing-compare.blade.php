...
@section('content')
    ...
    <header class="pt-32 pb-12 text-center bg-gradient-to-b from-brand-50 to-slate-50 dark:from-slate-900 dark:to-slate-900 transition-colors duration-300">
        <div class="container mx-auto px-6">
            <h1 class="text-4xl lg:text-5xl font-extrabold text-brand-dark dark:text-white mb-4">{{ __('gemini-landing.pricing_compare.title') }}</h1>
            <p class="text-lg text-slate-500 dark:text-slate-400 mb-8 max-w-2xl mx-auto">{{ __('gemini-landing.pricing_compare.subtitle') }}</p>
            ...
            <div class="flex flex-wrap justify-center gap-2 mb-10 p-1 bg-slate-200 dark:bg-slate-800 rounded-2xl max-w-fit mx-auto">
                <button onclick="switchSystem('pos')" id="btn-pos" class="system-tab active-tab px-8 py-3 rounded-xl font-bold transition-all duration-300">
                    <i class="fa-solid fa-cash-register mr-2"></i> {{ __('gemini-landing.pricing_compare.tabs.pos') }}
                </button>
                <button onclick="switchSystem('hrm')" id="btn-hrm" class="system-tab px-8 py-3 rounded-xl font-bold text-slate-500 transition-all duration-300">
                    <i class="fa-solid fa-users-gears mr-2"></i> {{ __('gemini-landing.pricing_compare.tabs.hrm') }}
                </button>
                <button onclick="switchSystem('booking')" id="btn-booking" class="system-tab px-8 py-3 rounded-xl font-bold text-slate-500 transition-all duration-300">
                    <i class="fa-solid fa-calendar-check mr-2"></i> {{ __('gemini-landing.pricing_compare.tabs.booking') }}
                </button>
            </div>

            <div class="flex items-center justify-center gap-4 mb-12">
                <span class="text-slate-600 dark:text-slate-300 font-semibold">{{ __('gemini-landing.pricing_compare.billing_monthly') }}</span>
                ...
                <span class="text-slate-400 dark:text-slate-500 font-semibold">{{ __('gemini-landing.pricing_compare.billing_yearly') }} <span class="text-brand-500 text-xs font-bold bg-brand-100 px-2 py-0.5 rounded-full ml-1">{{ __('gemini-landing.pricing_compare.discount_badge') }}</span></span>
            </div>
        </div>
    </header>
        ...
        <div class="container mx-auto px-6">
            ...
            <div class="max-w-5xl mx-auto bg-white dark:bg-slate-800 rounded-3xl shadow-sm overflow-hidden border border-slate-100 dark:border-slate-700">
                <div class="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-white" id="table-title">{{ __('gemini-landing.pricing_compare.table_default_title') }}</h2>
                </div>
...
@push('scripts')
    ...
    <script>
        ...
        const i18n = @json([
            'best_value' => __('gemini-landing.pricing_compare.best_value'),
            'per_month' => __('gemini-landing.pricing_compare.per_month'),
            'free_trial' => __('gemini-landing.pricing_compare.free_trial', ['months' => ':months']),
            'choose_plan' => __('gemini-landing.pricing_compare.choose_plan', ['plan' => ':plan']),
            'comparison_title' => __('gemini-landing.pricing_compare.comparison_title', ['title' => ':title']),
            'core_features' => __('gemini-landing.pricing_compare.core_features'),
        ]);
        ...
        function updateUI() {
            ...
            grid.innerHTML = data.plans.map(plan => `
                <div class="bg-white dark:bg-slate-800 rounded-2xl p-8 border-2 ${plan.popular ? 'border-brand-500 scale-105 shadow-xl' : 'border-transparent shadow-sm'} relative transition-all duration-300">
                    ${plan.popular ? `<div class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-500 text-white px-4 py-1 rounded-full text-xs font-bold uppercase">${i18n.best_value}</div>` : ''}
                    ...
                    <div class="flex items-baseline mb-6">
                        ...
                        <span class="text-slate-500 ml-1">${i18n.per_month}</span>
                    </div>
                    ${(plan.free_trial_months || 0) > 0 ? `<p class="text-xs text-green-600 dark:text-green-400 font-semibold mb-2">${i18n.free_trial.replace(':months', plan.free_trial_months)}</p>` : ''}
                    ...
                    <a href="${buildCheckoutUrl(activeSystem, plan, isYearly)}" class="block text-center w-full py-3 rounded-xl font-bold transition ${plan.popular ? 'bg-brand-500 text-white hover:bg-brand-600' : 'bg-slate-100 dark:bg-slate-700 dark:text-white hover:bg-slate-200'}">${i18n.choose_plan.replace(':plan', plan.name)}</a>
                </div>
            `).join('');

            // 2. Update Table
            document.getElementById('table-title').textContent = i18n.comparison_title.replace(':title', data.title);
            ...
            thead.innerHTML = `
                <tr class="text-slate-400 dark:text-slate-500 text-xs uppercase tracking-wider">
                    <th class="py-4 px-8 font-medium">${i18n.core_features}</th>
