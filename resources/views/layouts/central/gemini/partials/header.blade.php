...
<nav id="navbar" class="fixed w-full z-50 transition-all duration-300 py-4 bg-white/80 backdrop-blur-md dark:bg-slate-900/90 dark:border-b dark:border-slate-800">
    <div class="container mx-auto px-6 flex justify-between items-center">
        ...
        <div class="hidden lg:flex items-center gap-3">
            <div class="relative">
                ...
                <div id="lang-dropdown" class="absolute top-full end-0 mt-2 w-32 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 hidden overflow-hidden p-1 z-50">
            ...
            <button id="theme-toggle" class="w-10 h-10 rounded-full border border-slate-200 bg-white hover:border-brand-500 hover:text-brand-500 transition flex items-center justify-center text-slate-600 shadow-sm dark:bg-slate-800 dark:border-slate-700 dark:text-yellow-400">
                <i class="fa-regular fa-moon text-lg dark:hidden"></i>
                <i class="fa-solid fa-sun text-lg hidden dark:block"></i>
            </button>

            <a href="{{ route('pricing') }}" class="px-6 py-2.5 bg-brand-500 text-white rounded-full font-bold shadow-lg shadow-brand-500/30 hover:bg-brand-600 transition-all transform hover:-translate-y-0.5 ms-2">
                {{ __('gemini-landing.nav.get_started') }}
            </a>
        </div>

        <button id="mobile-menu-btn" class="lg:hidden text-2xl text-slate-800 dark:text-white focus:outline-none">
    ...
    <div id="mobile-menu" class="hidden absolute w-full top-full start-0 bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-slate-800 shadow-xl p-6 flex flex-col gap-4 lg:hidden">
        ...
        <div class="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4 mt-2">
            ...
            <a href="{{ route('site.lang', $anotherLang['code']) }}" class="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700">
                <img src="{{ $anotherLang['image'] }}" class="w-5 h-5 rounded-full">
                <span class="text-sm font-bold dark:text-white">{{ $anotherLang['name'] }}</span>
            </a>
        </div>

        <div class="flex items-center justify-between">
            <span class="text-sm font-bold text-slate-500 dark:text-slate-400">{{ __('gemini-landing.nav.appearance') }}</span>
            <button id="mobile-theme-toggle" class="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-yellow-400">
                <i class="fa-regular fa-moon dark:hidden"></i>
                <i class="fa-solid fa-sun hidden dark:block"></i>
                <span class="text-sm font-bold dark:text-white">{{ __('gemini-landing.nav.change_mode') }}</span>
            </button>
        </div>

        <a href="{{ route('pricing') }}" class="block text-center w-full px-6 py-3 bg-brand-500 text-white rounded-lg font-bold mt-2">
            {{ __('gemini-landing.nav.get_started') }}
        </a>
    </div>
</nav>
