...
<header
    class="z-40 flex h-16 items-center justify-between gap-4 border-b border-gray-200 bg-white px-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 lg:px-6">
    <div class="flex min-w-0 items-center gap-4">
        @if(!isset($withoutSidebar))
            <button type="button"
                class="rounded-md p-2 text-gray-500 transition hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"
    ...
    <div class="flex items-center gap-2 lg:gap-3">
        ...
        @if($branches->count() > 0)
            <div class="relative hidden md:block" x-data="{ open: false }" @click.away="open = false">
                <button type="button" @click="open = !open"
                    class="flex items-center gap-2 rounded-md border border-gray-300 px-3 py-2 text-sm font-medium text-gray-600 transition hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">
                    <i class="fa fa-building text-gray-400 dark:text-slate-500"></i>
                ...
                <div x-show="open" x-transition x-cloak
                    ...
                    <div class="max-h-60 overflow-y-auto p-2">
                        ...
                        @foreach($branches as $b)
                            <a href="{{ url('/admin/switch-branch/' . $b->id) }}"
                                class="mt-1 flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition {{ $currentBranch == $b->id ? 'bg-blue-50 font-semibold text-blue-600 dark:bg-blue-900/20 dark:text-blue-400' : 'text-gray-600 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700' }}">
        ...
        {{-- <button type="button" @click="rtl = !rtl"
            class="rounded-md border border-gray-300 px-3 py-1 text-xs font-bold transition hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-700">
        ...
        <div class="relative" x-data="{ open: false }" @click.away="open = false">
            <button type="button" @click="open = !open"
                class="relative rounded-full p-2 text-gray-500 transition hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700">
                @if(isset($__unreaded_notifications) && count($__unreaded_notifications) > 0)
                    <span
                        class="absolute right-2 top-2 h-2 w-2 rounded-full border-2 border-white bg-red-500 dark:border-gray-800"></span>
                @endif
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div x-show="open" x-transition x-cloak
                ...
                <div
                    class="sticky bottom-0 border-t border-gray-100 bg-white p-3 text-center dark:border-gray-700 dark:bg-gray-800">
                    <a href="{{ route('admin.notifications.list') }}"
                        class="text-sm font-semibold text-blue-600 transition hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">{{ __('general.layout.see_all') }}</a>
                </div>
            </div>
        </div>
        ...
        <div class="relative" x-data="{ open: false }" @click.away="open = false">
            ...
            <div x-show="open" x-transition x-cloak
                ...
                <a href="{{ route('admin.logout') }}"
                    class="flex items-center px-4 py-2 text-sm font-semibold text-red-500 transition hover:bg-gray-50 dark:hover:bg-gray-700">
