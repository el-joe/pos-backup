            ...
            <div class="space-y-4">
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="rounded-2xl border border-sky-200 bg-sky-50 p-4 dark:border-sky-500/20 dark:bg-sky-500/10">
                        <p class="text-xs font-medium uppercase tracking-[0.2em] text-sky-700 dark:text-sky-300">{{ __('general.pages.products.branch_stock') }}</p>
                        <p class="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{{ $product->branch_stock }}</p>
                    </div>
                    <div class="rounded-2xl border border-brand-200 bg-brand-50 p-4 dark:border-brand-500/20 dark:bg-brand-500/10">
                        <p class="text-xs font-medium uppercase tracking-[0.2em] text-brand-700 dark:text-brand-300">{{ __('general.pages.products.all_stock') }}</p>
                        <p class="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">{{ $product->all_stock }}</p>
                    </div>
                </div>
        ...
        <div class="p-5">
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>@forelse($stocks as $stock)<tr>
                            ...
                            <td colspan="5" class="py-10 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('general.pages.products.no_stock_records') }}</td>
                        </tr>@endforelse</tbody>
                </table>
            </div>
        </div>
        ...
        <div class="p-5">
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>@forelse($recentSalesItems as $item)<tr>
                            ...
                            <td class="text-nowrap">@if($item->sale)<a href="{{ route('admin.sales.details', $item->sale->id) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-sky-50 text-sky-600 transition hover:bg-sky-100 dark:bg-sky-500/10 dark:text-sky-300 dark:hover:bg-sky-500/20"><i class="fa fa-eye"></i></a>@endif</td>
                        </tr>@empty<tr>
                            <td colspan="6" class="py-10 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('general.pages.products.no_sales_records') }}</td>
                        </tr>@endforelse</tbody>
                </table>
            </div>
        </div>
        ...
        <div class="p-5">
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>@forelse($recentPurchaseItems as $item)<tr>
                            ...
                            <td class="text-nowrap">@if($item->purchase)<a href="{{ route('admin.purchases.details', $item->purchase->id) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-sky-50 text-sky-600 transition hover:bg-sky-100 dark:bg-sky-500/10 dark:text-sky-300 dark:hover:bg-sky-500/20"><i class="fa fa-eye"></i></a>@endif</td>
                        </tr>@empty<tr>
                            <td colspan="6" class="py-10 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('general.pages.products.no_purchase_records') }}</td>
                        </tr>@endforelse</tbody>
                </table>
            </div>
        </div>
        ...
        <div class="p-5">
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>@forelse($recentTransferItems as $item)<tr>
                            ...
                            <td colspan="6" class="py-10 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('general.pages.products.no_transfer_records') }}</td>
                        </tr>@endforelse</tbody>
                </table>
            </div>
        </div>
        ...
        <div class="p-5">
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>@forelse($recentAdjustments as $adj)<tr>
                            <td>{{ $adj->stock_taking_id }}</td>
                            <td>{{ $adj->stockTaking?->branch?->name }}</td>
                            <td>{{ round($adj->current_qty, 3) }}</td>
                            <td>{{ round($adj->actual_qty, 3) }}</td>
                            <td><span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold {{ $adj->difference >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300' }}">{{ round($adj->difference, 3) }}</span></td>
                            <td class="text-nowrap">@if($adj->stockTaking)<a href="{{ route('admin.stocks.adjustments.details', $adj->stock_taking_id) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-sky-50 text-sky-600 transition hover:bg-sky-100 dark:bg-sky-500/10 dark:text-sky-300 dark:hover:bg-sky-500/20"><i class="fa fa-eye"></i></a>@endif</td>
                        </tr>@empty<tr>
                            <td colspan="6" class="py-10 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('general.pages.products.no_adjustment_records') }}</td>
                        </tr>@endforelse</tbody>
                </table>
            </div>
        </div>
