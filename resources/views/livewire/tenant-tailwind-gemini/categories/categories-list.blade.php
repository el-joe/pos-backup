<div class="flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">
                    {{ __('general.pages.categories.search_by_name') }}
                </label>
                <input type="text"
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">
                    {{ __('general.pages.categories.parent_category') }}
                </label>
                <select class="select2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:focus:border-brand-500"
                    ...
                    @foreach ($allCategories as $cat)
                        <option value="{{ $cat->id }}" {{ ($filters['parent_id']??0) == $cat->id ? 'selected' : '' }}>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">
                    {{ __('general.pages.categories.status') }}
                </label>
                <select class="select2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:focus:border-brand-500"
                        name="filters.active">
            ...
            <div class="col-span-1 flex items-end justify-start md:col-span-2 lg:col-span-3 xl:col-span-1 xl:justify-end mt-2">
                <button type="button"
    ...
    <x-tenant-tailwind-gemini.table-card
        :title="__('general.pages.categories.categories')"
        description="Organize product hierarchies, parent relationships, and icon metadata."
            ...
            <div class="flex flex-wrap items-center gap-2">
                @adminCan('categories.export')
                    <button class="inline-flex items-center gap-2 rounded-xl border border-emerald-500 bg-white px-3 py-1.5 text-sm font-medium text-emerald-600 transition-colors hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-1 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-400 dark:hover:bg-emerald-500/20 dark:focus:ring-offset-slate-900"
                        ...
                        <i class="fa fa-file-excel"></i> {{ __('general.pages.categories.export') }}
                    </button>
                @endadminCan
                ...
                @adminCan('categories.create')
                    <button class="inline-flex items-center gap-2 rounded-xl bg-brand-500 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-1 dark:focus:ring-offset-slate-900"
                            ...
                            wire:click="$dispatch('category-set-current', null)">
            ...
            <table class="w-full whitespace-nowrap text-left text-sm text-slate-600 dark:text-slate-400">
                <thead class="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-800/50 dark:text-slate-300">
                    <tr>
                        ...
                        <th class="px-5 py-3 font-semibold">{{ __('general.pages.categories.name') }}</th>
                        <th class="px-5 py-3 font-semibold">{{ __('general.pages.categories.parent_category') }}</th>
                        <th class="px-5 py-3 font-semibold">{{ __('general.pages.categories.icon') }}</th>
                        <th class="px-5 py-3 font-semibold">{{ __('general.pages.categories.status') }}</th>
                        <th class="px-5 py-3 text-right font-semibold">{{ __('general.pages.categories.action') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 border-t border-slate-100 dark:divide-slate-800 dark:border-slate-800">
                    @forelse ($categories as $category)
                        <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                            <td class="px-5 py-4">{{ $category->id }}</td>
                            <td class="px-5 py-4 font-medium text-slate-900 dark:text-white">{{ $category->name }}</td>
                            <td class="px-5 py-4">{{ $category->parent ? $category->parent->name : __('general.pages.categories.n_a') }}</td>
                            <td class="px-5 py-4">
                                @if($category->icon)
                                    <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                                        <i class="{{ $category->icon }}"></i>
                                        {{ $category->icon }}
                                    </span>
                                @else
                                    <span class="text-slate-400 dark:text-slate-500">{{ __('general.pages.categories.n_a') }}</span>
                                @endif
                            </td>
                            <td class="px-5 py-4">
                                <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium {{ $category->active ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400' }}">
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center">
                                <div class="mx-auto max-w-sm space-y-2">
                                    <div class="text-base font-medium text-slate-900 dark:text-white">No categories found.</div>
