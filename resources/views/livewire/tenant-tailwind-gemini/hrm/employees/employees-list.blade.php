        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
            <div>
                ...
                <input type="text"
                       ...
                       placeholder="{{ __('general.pages.hrm.search_employee_placeholder') }}"
        ...
        <table class="w-full whitespace-nowrap text-left text-sm text-slate-600 dark:text-slate-400">
            <thead class="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-800/50 dark:text-slate-300">
                <tr>
                    ...
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.hrm.code') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.hrm.name') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.hrm.email') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.hrm.department') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.hrm.designation') }}</th>
            ...
            <tbody class="divide-y divide-slate-100 border-t border-slate-100 dark:divide-slate-800 dark:border-slate-800">
                @forelse($employees as $e)
                    <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        <td class="px-5 py-4">{{ $e->id }}</td>
                        <td class="px-5 py-4 font-mono text-xs text-slate-500 dark:text-slate-400">{{ $e->employee_code }}</td>
                        <td class="px-5 py-4 font-medium text-slate-900 dark:text-white">{{ $e->name }}</td>
                        <td class="px-5 py-4">{{ $e->email ?? '-' }}</td>
                        <td class="px-5 py-4">{{ $e->department?->name ?? '-' }}</td>
                        <td class="px-5 py-4">{{ $e->designation?->title ?? '-' }}</td>
                        <td class="px-5 py-4">{{ $e->manager?->name ?? '-' }}</td>
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                                {{ $e->status?->label() ?? '-' }}
                            </span>
                        </td>
                            ...
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @adminCan('hrm_master_data.create')
                                    <button class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-brand-600 transition-colors hover:bg-brand-50 focus:outline-none focus:ring-2 focus:ring-brand-500/50 dark:text-brand-400 dark:hover:bg-brand-500/10 dark:focus:ring-brand-400/50"
                                            ...
                                            title="{{ __('general.pages.hrm.new_replace_contract') }}">
                ...
                @empty
                    <tr>
                        <td colspan="9" class="px-5 py-8 text-center text-sm text-slate-500 dark:text-slate-400">
                            {{ __('general.pages.hrm.no_data_found') ?? 'No employees found.' }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
