<div class="space-y-6">
    <x-tenant-tailwind-gemini.table-card :title="__('general.pages.refunds.refund_details') . ' #' . $refund->id" :description="$refund->created_at?->format('Y-m-d H:i')" icon="fa-rotate-left">
        ...
        <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 md:col-span-2 xl:col-span-4 dark:border-slate-800 dark:bg-slate-900/60">
        ...
        <table class="w-full whitespace-nowrap text-left text-sm text-slate-600 dark:text-slate-400">
            <thead class="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-800/50 dark:text-slate-300">
                <tr>
                    ...
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.refunds.product') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.refunds.unit') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.refunds.qty') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.refunds.refundable') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 border-t border-slate-100 dark:divide-slate-800 dark:border-slate-800">
                @forelse($refund->items as $item)
                    <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        ...
                        <td class="px-5 py-4 font-medium text-slate-900 dark:text-white">{{ $item->product?->name ?? '—' }}</td>
                        <td class="px-5 py-4">{{ $item->unit?->name ?? '—' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-8 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('general.pages.refunds.no_items') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <x-tenant-tailwind-gemini.table-card :title="__('general.pages.refunds.transactions')" icon="fa-receipt" :render-table="false">
        <table class="w-full whitespace-nowrap text-left text-sm text-slate-600 dark:text-slate-400">
            <thead class="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-800/50 dark:text-slate-300">
                <tr>
                    <th class="px-5 py-3 font-semibold">#</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.refunds.date') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.refunds.type') }}</th>
                    <th class="px-5 py-3 font-semibold">{{ __('general.pages.refunds.description') }}</th>
                    <th class="px-5 py-3 text-right font-semibold">{{ __('general.pages.refunds.amount') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 border-t border-slate-100 dark:divide-slate-800 dark:border-slate-800">
                @forelse($transactions as $transaction)
                    <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        <td class="px-5 py-4">{{ $transaction->id }}</td>
                        <td class="px-5 py-4">{{ $transaction->date ?? $transaction->created_at?->format('Y-m-d') }}</td>
                        <td class="px-5 py-4">{{ $transaction->type?->label() ?? (string)$transaction->type }}</td>
                        <td class="px-5 py-4">{{ $transaction->description }}</td>
                        <td class="px-5 py-4 text-right font-medium text-slate-900 dark:text-white">{{ currencyFormat($transaction->amount ?? 0, true) }}</td>
                    </tr>
                    <tr>
                        <td colspan="5" class="bg-slate-50 px-5 py-4 dark:bg-slate-800/40">
                            ...
                            <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
                                <table class="min-w-full divide-y divide-slate-200 text-left text-sm dark:divide-slate-700">
                                    <thead class="bg-slate-50 text-xs uppercase text-slate-700 dark:bg-slate-800/60 dark:text-slate-300">
                                        <tr>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-8 text-center text-sm text-slate-500 dark:text-slate-400">{{ __('general.pages.refunds.no_transactions') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
