...
<div class="flex flex-col gap-6">
    ...
    <x-tenant-tailwind-gemini.collapsed-card :title="__($titleKey)" :icon="$icon" :expanded="$collapseFilters">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
            <x-tenant-tailwind-gemini.form-group :label="$T('search')" class="md:col-span-3">
                <x-tenant-tailwind-gemini.search-input :placeholder="$T('search_placeholder')"
                        ...
                        <x-tenant-tailwind-gemini.btn-group align="end">
                            @foreach(($statusActions ?? []) as $sa)
                                ...
                                @if($canShow)
                                    ...
                                    <x-tenant-tailwind-gemini.icon-btn :icon="$sa['icon'] ?? 'fa fa-check'" :tone="$tone" size="sm"
                            ...
                            <x-tenant-tailwind-gemini.icon-btn icon="fa fa-pencil" tone="blue" size="sm" :title="$T('edit')"
                                wire:click="edit({{ $row->id }})" />
                            @endadminCan
                            @adminCan($permission . '.delete')
                            <x-tenant-tailwind-gemini.icon-btn icon="fa fa-times" tone="rose" size="sm"
    ...
    <x-tenant-tailwind-gemini.modal :title="__($titleKey)" :icon="$icon" size="lg" openEvent="contracting-open-modal"
            ...
            <div class="max-h-[65vh] overflow-y-auto px-6 py-5">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    @foreach($fieldsMeta as $field)
                        ...
                        <div class="{{ $colSpan }}">
                            @if($type === 'boolean')
                                <x-tenant-tailwind-gemini.checkbox :label="$label($field['label_key'])"
                                    wire:model.blur="form.{{ $f }}" />
                                @if($fieldError)
                                    <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $fieldError }}</p>
                                @endif
                            @else
                                <x-tenant-tailwind-gemini.form-group :label="$label($field['label_key'])"
                                    :required="!empty($field['required'])" :error="$fieldError">

                                    @if($type === 'textarea')
                                        <x-tenant-tailwind-gemini.textarea :invalid="(bool) $fieldError"
                                            wire:model.blur="form.{{ $f }}" />
                                    @elseif($type === 'select')
                                        <x-tenant-tailwind-gemini.select :invalid="(bool) $fieldError"
                                            wire:model.blur="form.{{ $f }}" :placeholder="'— ' . $T('select') . ' —'">
                                            @foreach(($field['options'] ?? []) as $k => $v)
                                                <option value="{{ $k }}">
                                                    {{ is_string($v) && str_starts_with($v, 'general.') ? __($v) : $v }}</option>
                                            @endforeach
                                    ...
                                    @elseif($type === 'date')
                                        <x-tenant-tailwind-gemini.input type="date" :invalid="(bool) $fieldError"
                                            wire:model.blur="form.{{ $f }}" />
                                    @elseif($type === 'decimal' || $type === 'number')
                                        <x-tenant-tailwind-gemini.input type="number" :invalid="(bool) $fieldError"
                                            step="{{ $type === 'decimal' ? '0.01' : '1' }}" wire:model.blur="form.{{ $f }}" />
                                    @else
                                        <x-tenant-tailwind-gemini.input :type="$type === 'email' ? 'email' : 'text'" :invalid="(bool) $fieldError" wire:model.blur="form.{{ $f }}" />
                                    @endif
                                </x-tenant-tailwind-gemini.form-group>
                            @endif
                        </div>
