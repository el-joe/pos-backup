    ...
    <x-tenant-tailwind-gemini.table-card :title="$user->name . ' - ' . __('general.pages.users.profile')" icon="fa fa-user">
        ...
        <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
                <p class="text-xs font-medium uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{{ __('general.pages.users.full_name') }}</p>
                <p class="mt-2 font-semibold text-slate-900 dark:text-white">{{ $user->name }}</p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
                <p class="text-xs font-medium uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{{ __('general.pages.users.mobile') }}</p>
                <p class="mt-2 font-semibold text-slate-900 dark:text-white">{{ $user->phone }}</p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
                <p class="text-xs font-medium uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{{ __('general.pages.users.email') }}</p>
                <p class="mt-2 font-semibold text-slate-900 dark:text-white">{{ $user->email }}</p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
                <p class="text-xs font-medium uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{{ __('general.pages.users.address') }}</p>
                <p class="mt-2 font-semibold text-slate-900 dark:text-white">{{ $user->address }}</p>
            </div>
        </div>
