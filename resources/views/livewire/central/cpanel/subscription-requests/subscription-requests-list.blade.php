        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>
                        @foreach ($subscriptionRequests as $req)
                            <tr>
                                ...
                                <td>
                                    @if($req->pay_from_balance)
                                        <span class="badge bg-primary">Wallet Balance</span>
                                    @elseif($req->manual)
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Subscription Request #{{ $current->id }}</h5>
                    ...
                    <table class="table table-bordered">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $current->plan?->name }} ({{ $current->billing_cycle }})</td>
                            </tr>
                            <tr>
                                <th class="bg-light">Amount (USD)</th>
                                <td>${{ number_format((float) $current->price, 2) }}</td>
                            </tr>
                            <tr>
                                <th class="bg-light">Payment Method</th>
                                <td>
                                    @if($current->pay_from_balance)
                                        <span class="badge bg-primary px-2 py-1 fs-6">Wallet Balance</span>
                                    @else
                                        <span class="badge bg-primary px-2 py-1 fs-6">{{ $current->paymentMethod?->name }}</span>
                                    @endif
                                </td>
                            </tr>
                            @if($current->manual)
                            <tr>
                                ...
                                <td>
                                    <span class="badge border border-dark text-dark px-2 py-1"><i class="bi bi-cash me-1"></i> Manual Transfer</span>
                                </td>
                            </tr>
                            @endif
